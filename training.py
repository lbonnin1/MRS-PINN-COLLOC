...
"""coloc1=torch.linspace(4356, 4420,50).unsqueeze(1).unsqueeze(1)
coloc2=torch.linspace( 4630, 4784,120).unsqueeze(1).unsqueeze(1)
coloc3=torch.linspace(5114, 5207,90).unsqueeze(1).unsqueeze(1)
X_collocation = torch.cat((coloc1, coloc2, coloc3), dim=0)"""
collocation_range = torch.arange(4761,5981, 100)
best_loss=0
best_coloc=None
...
for X_collocation in collocation_range:
    ...
    for epoch in range(epochs):
        ...
        for batch_index, batch in enumerate(loader_train):
            ...
            loss_boundaries = normalized_loss_amplitude + normalized_loss_Width + normalized_loss_type
            loss_coloc, loss_physics=loss_pinn(X_collocation, amplitude, width, reconstructed_spectrum,type )
            loss_total=loss_boundaries+torch.tensor(loss_coloc)+torch.tensor(loss_physics)
        ...
        plt.tight_layout()
        plt.savefig(f'plots2/true_vs_predicted_epoch_{epoch}.png')
        plt.close()"""
        ...
        # Plot the correlation matrix
            ...
            os.makedirs('corr_final_ablation_AW', exist_ok=True)
            plt.savefig(f'corr_final_ablation_AW/predicted_correlation_epoch_{epoch}.png')
            plt.close()
        """
        ...
        with torch.no_grad():  # Disable gradient computation
            ...
            for batch in loader_val:
                ...
                val_loss_boundaries =  val_loss_amplitude + val_loss_Width +val_loss_type
                val_loss_coloc,val_loss_physics=loss_pinn(X_collocation, amplitude, width, reconstructed_spectrum,type )
                val_loss_total=torch.tensor(val_loss_coloc)+val_loss_boundaries+torch.tensor(val_loss_physics)
             
            print(f"epoch{epoch + 1} ")
            """print(f"Loss Amplitude: {normalized_loss_amplitude.item():.4f}, "
            f"Loss Width: {normalized_loss_Width.item():.4f}, "
            f"Loss Type: {normalized_loss_type.item():.4f}, "
           
            f"Loss coloc: {loss_coloc.item():.4f}, "
    ...
    if loss> best_loss:
            best_loss=loss
            best_coloc=X_collocation
    

    with open('results.txt', 'a') as f:
        # Write each score and MSE to the file
        f.write(f"X_collocation: {X_collocation}\nR2 Scores: {mean_r2_reshaped[-1]}\nMSE: {mean_mse_reshaped[-1]}\nloss:{loss/13}")
